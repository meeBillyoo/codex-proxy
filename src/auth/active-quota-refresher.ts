 ...
 * Periodically refreshes the current account when it is locked or dirty.
 */
...
export class ActiveQuotaRefresher {
  ...
  private async tick(): Promise<void> {
    try {
      const now = Date.now();
      const entry = this.pool.getCurrentEntry();
      if (entry?.status === "active") {
        ...
        if (now - this.lastRefreshedAt < this.resolveIntervals().minGapMs) return;
 ...
 * account appears available again and the next real request wastes a full
 * payload upload before being 429'd again.
 *
