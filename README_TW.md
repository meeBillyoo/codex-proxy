...
可以使用 No-Node Lite。它包含後端與控制面板，但不內建 Node.js，
因此套件更小，也方便您自行管理執行環境。
...
此版本需要 Node.js 24 或更新版本。Windows 會優先使用 WebView2；如果無法使用，則啟動服務並
...
# 優先使用 Node.js 24；目前主版本不是 24 時，透過 nvm 安裝並切換
...
原始碼預設設定僅監聽 `127.0.0.1`，除非 `data/local.yaml` 明確覆蓋。如需區域網路內其他設備造訪，可在 `data/local.yaml` 中設定：
...
- **Node.js** 24+
- **Rust** — 原始碼運行需 Rust 工具鏈（編譯 TLS native addon）
- **ChatGPT 帳號** — 免費帳號即可
